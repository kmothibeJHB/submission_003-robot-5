...
if len(sys.argv) > 1 and sys.argv[1] == 'turtle':
    ...
    from world.turtle import world as some
    turtle_map()
else:
    from world.text.world import *
    from world.text import world as some
...
# Globalise the robot name to be used in other modules
robot_name = ""
#commands history
history = []
print_obstacles = obstacles.print_obstacles()

def get_robot_name():
    """
    gets the robot name from user input
    """
...
def output(name, message):
    """
    print function that prints robot name and the required display text.
    """
    print(''+name+": "+message)
...
def do_help():
    """
    Provides help information to the usercurrent_direction_index = 0
...
def get_commands_history(reverse, relativeStart, relativeEnd):
    ...
    range_start = len(commands_to_replay) + relativeStart if (relativeStart\
        is not None and (len(commands_to_replay) + relativeStart) >= 0) else 0
    range_end = len(commands_to_replay) + relativeEnd\
        if  (relativeEnd is not None and (len(commands_to_replay) + relativeEnd)\
            >= 0 and relativeEnd > relativeStart) else len(commands_to_replay)
...
def do_replay(robot_name, arguments):
    ...
    return True, ' > '+robot_name+' replayed ' + str(len(commands_to_replay)) +\
        ' commands' + (' in reverse' if reverse else '') + (' silently.' if silent else '.')


def call_command(command_name, command_arg, robot_name):
    """
    calls all the valid commands
    """
...
def robot_start():
    ...
    history = []
    some.current_direction_index = 0
    obstacles.current_direction_index = 0
    some.position_x, some.position_y = 0, 0
    some.new_x,some.new_y = 0,0
